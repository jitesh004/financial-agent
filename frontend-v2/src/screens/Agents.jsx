...
import { ago, dateLabel, duration } from '../core/format';
import { Button, Callout, Card, Chip, ConfirmButton, Empty, Icon, Loading, Table } from '../ui';
import JobProgress from '../ui/JobProgress';

const ICONS = { scale: 'scales', drip: 'trending', wave: 'gauge', receipt: 'file', shield: 'shield' };
    ...
    <>
      ...
      {historyAgent ? (
        ...
        <div className="grid cols-2">
          {agents.map((agent) => (
            <div key={agent.key} className="col" style={{ gap: 8 }}>
              <AgentCard
                agent={agent}
                disabled={blocked}
              ...
              <input
                ...
                onChange={(e) => setQuestion((p) => ({ ...p, [agent.key]: e.target.value }))}
              ...
              <tr key={run.id}>
                ...
                <td>
                  <button type="button" className="drill" style={{ textAlign: 'left' }}
                    ...
                    {run.answer?.headline || <em>{run.error || 'no answer'}</em>}
                  </button>
                </td>
...
function AgentAnswer({ run, onBack }) {
  ...
  return (
    <>
      ...
      {failed && <Callout tone="neg">This run did not finish: {run.error}</Callout>}
      {exhausted && (
        <Callout tone="warn">
      ...
      {answer.caveats?.length > 0 && (
        <Callout tone="warn">
          ...
          <ul style={{ margin: '6px 0 0', paddingLeft: 18 }}>
            {answer.caveats.map((c, i) => <li key={i} style={{ listStyle: 'disc' }}>{c}</li>)}
          </ul>
        </Callout>
      )}
